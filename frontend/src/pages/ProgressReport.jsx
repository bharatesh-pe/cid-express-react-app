...
const ProgressReport = ({templateName, headerDetails, rowId, options, selectedRowData, backNavigation}) => {
       ...
       useEffect(() => {
        const fetchData = async () => {
            if (rowId) {
                await checkPdfEntryStatus(rowId); // Ensure this finishes first
                getUploadedFiles(rowId, options); // Then fetch files
            }
        };
...
const toggleSelectRow = (id) => {
  console.log("Toggling row selection for id:", id); // Log selected row ID for debugging
  setSelectedIds((prevSelectedIds) => {
    const updated = prevSelectedIds.includes(id)
      ? prevSelectedIds.filter((selectedId) => selectedId !== id)
      : [...prevSelectedIds, id];

    console.log("Updated selectedIds:", updated); // Log updated selectedIds array
    return updated;
  });
};
    ...
    if (getTemplateResponse && getTemplateResponse.success) {
      ...
      if (records[0]) {
        const excludedKeys = [
          ...
          "linked_profile_info", "sys_status", "field_status", "field_submit_status",
...
const handleUpdatePdfClick = async ({
  ...
  try {
    ...
    console.log("Template Data Response:", getTemplateResponse);
    ...
    if (getTemplateResponse && getTemplateResponse.success) {
      ...
      console.log("AO Field Values:", aoFieldValues);
      ...
      console.log("Filtered AO Field Values:", filteredAOFieldValues);
...
const prUpdatePdf = async ({ appendText, aoFields, selectedIds, selectedRow }) => {
  try {
    console.log("Updating PDF with appendText:", appendText);
    console.log("AO Fields:", aoFields);
    ...
    const payload = {
      ...
      appendText,           // <-- already an object/array, DO NOT parse
      aoFields,             // <-- already an object, DO NOT parse
...
{hasPdfEntry && (
  <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
    ...
    >
      {/* Optional empty box or insert content if needed */}
...
>
  ...
  >
    Submit
                ...
                {!hasPdfEntry ? (
                // Show Upload Prompt if PDF is not found
                <Box
                    display="flex"
                    flexDirection="column"
                    alignItems="center"
                ...
                {selectedTab === 2 && (  // Monthwise Table Tab
                    <Box>
                    <TableView
                        rows={monthwiseData}
