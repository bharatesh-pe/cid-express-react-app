...
const MastersView = () => {
    ...
    useEffect(() => {
        ...
        if (!showMasters) {  // Prevent running when showMasters is true
            loadTableData(paginationCount);
        }
    }, [paginationCount, tableSortKey, tableSortOption, starFlag, readFlag, showMasters]); 
    
    const loadTableData = async (page, searchValue, templateModule = selectedTemplateModule) => {
        ...
        try {
            ...
            if (getTemplateResponse && getTemplateResponse.success) {
                if (getTemplateResponse.data && getTemplateResponse.data['data']) {
                    console.log("Columns Array:", getTemplateResponse.data.columns);
                    console.log("Data Object:", getTemplateResponse.data['data'][0]);
                ...
                try {
                    ...
                    if (deleteTemplateDataResponse && deleteTemplateDataResponse.success) { 
                        toast.success(deleteTemplateDataResponse.message ? deleteTemplateDataResponse.message : "Template Deleted Successfully", {
                            ...
                            onOpen: () => {
                                setSelectedTemplateModule(prevModule => {
                                    const updatedModule = prevModule || selectedTemplateModule;
                                    loadTableData(paginationCount, "", updatedModule);
    ...
    const handleSelectChange = (name, selectedName) => {
        ...
        if (selectedMasterObj) {
            ...
            loadTableData(1, "", selectedMasterObj.key);
        }
    };
        
            
    const backMastersList = () => {
        setShowMasters(true);
    }
    ...
    return (
        <Box p={2} inert={loading ? true : false}>
            ...
            {showMasters ? (
            <Box>
                <Typography className='Roboto' sx={{ fontSize: '20px', fontWeight: '600', color: '#1D2939', marginBottom: '15px' }}>
                    Masters
                </Typography>
                <AutocompleteField
                    formData={{ master: selectedMaster }}
                    errors={{}}
                    field={{
                        name: "master",
                        label: "Select Master",
                    ...
                    onChange={handleSelectChange} // This now receives (name, value)
                    // isFocused={Boolean(selectedMaster)}
                />
            </Box>
        ) : (       
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
                        <Box sx={{ display: 'inline-flex', alignItems: 'center', cursor: 'pointer' }}>
                            <img src='../arrow-left.svg' onClick={backMastersList} />
                            <Typography variant="h1" align="left" className='ProfileNameText'>
                        {console.log("template_nametemplate_name",template_name)}
                                {template_name ? template_name.toLowerCase().replace(/^\w|\s\w/, (c) => c.toUpperCase()) : 'Master'}
                            </Typography>
                        </Box>
                    </Box>
