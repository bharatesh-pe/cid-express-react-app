...
const UnderInvestigation = () => {
    ...
    return (
        <Box p={2} inert={loading ? true : false}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
                        <Box sx={{ display: 'inline-flex', alignItems: 'center', cursor: 'pointer' }}>
                            ...
                            <Typography variant="h1" align="left" className='ProfileNameText'>
                                {template_name ? template_name.toLowerCase().replace(/^\w|\s\w/, (c) => c.toUpperCase()) : 'Under Investigation'}
                            </Typography>
                        </Box>
                    </Box>
            ...
            {isDownloadPdf && <GenerateProfilePdf is_print={isPrint} templateData={downloadPdfData} templateFields={downloadPdfFields} template_name={template_name} onSave={handleOnSavePdf} />}
