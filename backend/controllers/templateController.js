...
exports.createTemplate = async (req, res, next) => {
  ...
  try {
    ...
    return adminSendResponse(res, 400, false,  error.message || "Server error." , error);
  } finally {
    if (fs.existsSync(dirPath))
...
exports.deleteTemplate = async (req, res, next) => {
  ...
  try {
    ...
    return adminSendResponse(res, 500, false, error.message || "Server error.", error.message);
  } finally {
    if (fs.existsSync(dirPath))
      fs.rmSync(dirPath, { recursive: true, force: true });
  }
...
exports.getMasterTemplates = async (req, res, next) => {
  try {
    ...
    return adminSendResponse(res, 500, false, error.message ||"Server error.", error);
  }
};
