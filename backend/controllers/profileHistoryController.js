  ...
  try {
    ...
    if (search) {
      whereCondition[Op.or] = [
        ...
        Sequelize.where(
          Sequelize.fn('to_char', Sequelize.col('created_at'), 'YYYY-MM-DD HH24:MI:SS'),
          { [Op.iLike]: `%${search}%` }
        )
      ];
    }

    const total = await CaseHistory.count({ where: whereCondition });

    const caseLogs = await CaseHistory.findAll({
    ...
    const formattedLogs = caseLogs.map(log => ({
      type: "case_history",
      ...log.toJSON()
    }));

    return userSendResponse(res, 200, true, "Case History retrieved successfully.", {
      total,
